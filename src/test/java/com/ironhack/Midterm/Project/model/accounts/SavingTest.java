...
class SavingTest {
    ...
    void setBalance_GoodBalance_Balance() {
        assertEquals(BigDecimal.valueOf(2000000), saving.getBalance().getBalance());
        saving.getBalance().setBalance(BigDecimal.valueOf(1244657));
        assertEquals("The balance has been set", saving.setBalance(saving.getBalance()));
    }
    @Test
    void setBalance_Balance_Penalty() {
        saving.getBalance().setBalance(BigDecimal.valueOf(950));
        assertEquals("A penalty has been taken from the balance", saving.setBalance(saving.getBalance()));
        assertEquals(BigDecimal.valueOf(910), saving.getBalance().getBalance());
    }
    ...
    void getBalance_LastActualizedDateLess1month(){
        ...
        assertEquals(BigDecimal.valueOf(19000), saving3.getBalance().getBalance());
    }
    @Test
    void getBalance_LastActualizedDateMore1month(){
        Money money3 = new Money(BigDecimal.valueOf(19000));
        Saving saving5 = new Saving(money3, "fngmhg_fhª", user1, Date.valueOf("2020-05-03"));
        assertEquals(BigDecimal.valueOf(19047.50).setScale(2), saving5.getBalance(saving5.getLastActualizedDate()).getBalance());
        saving5.getBalance(saving5.getLastActualizedDate());
        assertEquals(Date.valueOf("2022-05-03"), saving5.getLastActualizedDate());
    }
}